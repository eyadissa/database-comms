...
func AddUser(d Userdata) int {
	...
	userID := exists(d.Username) //checking if the rand gen username happens to already exist
	if userID != -1 {
	...
	_, err = db.Exec(insertStatement, d.Username) //network executes the insert command in the database
	if err != nil {
		fmt.Println(err)
		return -1
	}

	//if the username exists return the user ID (an integer for indexing differnt than username)
	userID = exists(d.Username)
	if userID == -1 {
		return userID
	}

	//execute a network command the updated the full data fields in userdata
	...
	return userID //if we run without errors, return the new user id
}
